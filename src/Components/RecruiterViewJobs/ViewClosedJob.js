...
export default function ViewClosedJob({
  ...
  useEffect(() => {
    navigate(`/dashboard/jobs/closed?page=${page || 1}`);
    dispatch(setFetchAJobLoading());
    axios
      .get(
        `/api/recruiter/job?type=referred-closed&page=${page}&limit=${itemsPerPage}`,
        {
          headers: {
  ...
  const handlePageChange = (pageNumber) => {
    navigate(`/dashboard/jobs/closed?&page=${pageNumber}`);
  };

  const handleNextPage = () => {
    if (page < totalPages) {
      navigate(`/dashboard/jobs/closed?&page=${parseInt(page) + 1}`);
    }
  };
