...
export default function ViewMarkedJob({
  status,
  user,
  state,
  ...
  useEffect(() => {
    navigate(`/dashboard/jobs/marked?page=${page || 1}`);
    dispatch(setFetchAJobLoading());
    axios
      .get(
        `/api/recruiter/job?type=marked&page=${page}&limit=${itemsPerPage}`,
        {
          headers: {
      ...
      .catch((err) => {
        console.log(err);
        if (err.response.data.message == "No jobs found.") {
          setData([]);
          dispatch(setFetchAJobSuccess());
  ...
  const handlePageChange = (pageNumber) => {
    navigate(`/dashboard/jobs/marked?&page=${pageNumber}`);
  };

  const handleNextPage = () => {
    if (page < totalPages) {
      navigate(`/dashboard/jobs/marked?&page=${parseInt(page) + 1}`);
    }
  };
